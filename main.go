...
func main() {
	...
	if isValidName && isValidEmail && isValidTicketNumber {
		...
		if remainingTickets == 0 {
			fmt.Println("The go confrence is booked out come back next year, Thank you")
			// break
		}
	} else {
		if !isValidName {
			fmt.Println("First name or lastname is too short")
		}
...
func sendTicket(userTickets uint, firstName string, lastName string, email string) {
	...
	var ticket = fmt.Sprintf("%v tickets for %v %v %v", userTickets, firstName, lastName, email)
	fmt.Println("##########")
